...
public class MainController {
	...
	public static void main(String[] args) throws IOException {
		...
		members[1] = m2;
		members[2] = m3;
		//출력: MemberService 클래스를 사용하는데 ms의 레퍼런스를 가르킨다
		MemberService ms = new MemberService();
		ms.printMembers(members);
		/* pom.xml  dependency 마지막부분 서버시간 가져온 후 maven update 
		...
		try {
			timeClient.open();
			//throw new RuntimeException(); // 강제 오류 예외 Exception 처리
		} catch (SocketException e) {
			System.out.println("open메서드 실행 오류");
			e.printStackTrace();
		} finally {
			System.out.println("이 부분은 성공| 오류에 상관없이 실행됩니다");
		}
